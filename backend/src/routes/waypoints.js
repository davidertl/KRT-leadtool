...
router.post('/', requireAuth, async (req, res, next) => {
  try {
    ...
    // Get team_id for broadcast
    const unit = await query('SELECT team_id FROM units WHERE id = $1', [unit_id]);
    if (unit.rows[0]) {
      broadcastToTeam(unit.rows[0].team_id, 'waypoint:created', result.rows[0]);
    }
...
router.delete('/:id', requireAuth, async (req, res, next) => {
  try {
    ...
    const unit = await query('SELECT team_id FROM units WHERE id = $1', [result.rows[0].unit_id]);
    if (unit.rows[0]) {
      broadcastToTeam(unit.rows[0].team_id, 'waypoint:deleted', { id: result.rows[0].id });
    }
...
router.delete('/unit/:unit_id', requireAuth, async (req, res, next) => {
  try {
    ...
    const unit = await query('SELECT team_id FROM units WHERE id = $1', [req.params.unit_id]);
    if (unit.rows[0]) {
      broadcastToTeam(unit.rows[0].team_id, 'waypoints:cleared', { unit_id: req.params.unit_id });
    }
